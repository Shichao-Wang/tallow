...
class BufferShuffledDataset(dataset.Dataset[T_co]):
    def __init__(self, dataset: dataset.Dataset, buffer_size: int) -> None:
        super().__init__()
        self._dataset = dataset
    ...
    def __iter__(self) -> Iterator[T_co]:
        ...
        for item in self._dataset:
            if len(buffer) == self._buffer_size:
                idx = random.randrange(0, self._buffer_size)
                yield buffer[idx]
                buffer[idx] = item
            else:
                buffer.append(item)
...
class ToTensorDataset(TransformDataset):
    def __init__(
        self, dataset: dataset.Dataset[T_co], pin_memory: bool = True
    ) -> None:
        super().__init__(dataset, partial(to_tensor, pin_memory=pin_memory))
